import argparse
from align.back import dotdict, get_timestamp_other, mkdir
import json
import copy
import os
import align.back as back
...
def dump_parameters(args):
    ...
    with open(os.path.join(args.config_dir, args.exp_name + ".json"), 'w') as outfile:
        if not (type(args) is dict or type(args) is back.dotdict):
...
def get_parser():
    ...
    parser.add_argument('--ensemble-step', type=float, default=0, action='store', help='rate of adjustment towards the second model')
