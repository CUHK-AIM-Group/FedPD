...
from align import parameters, align_models
...
def communication(args, server_model, models, client_weights):
    with torch.no_grad():
        ...
        elif args.mode.lower() == 'fedpd':
            for j in models:
                j = j.cpu()
            for i in range(len(models) - 1):
                # print(i)
                models[i+1] = align_models.get_wassersteinized_layers_modularized(args, [models[i+1], models[0]])
            for j in models:
                j = j.cuda()
            for key in server_model.state_dict().keys():
    ...
    for i in range(args.max_epoch):
        ...
        for client_idx in range(client_num):
            ...
            if i==args.max_epoch-1:
                torch.save(models[client_idx].state_dict(), os.path.join(log_path, 'models', '{}.checkpoint'.format(client_idx)))
