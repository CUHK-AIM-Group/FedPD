...
# Generate parameter mask
def LTH(model, keep_ratio = 0.999, eps = 1e-10):
    grads = dict()
...
def traindummy(args, trainloader, net, optimizer, criterion):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        (0.01*loss1+args.lamda2*loss3).backward(retain_graph=True)
        open_sub = LTH(net)  
        # for i, name in enumerate(metric):
        #     if i==0:
        #         open_metric = torch.mean(metric[name], dim=(1, 2, 3))

        optimizer.zero_grad()
        (args.lamda1*loss2).backward(retain_graph=True)
        close_sub = LTH(net)
        # for i, name in enumerate(metric):
        #     if i==0:
        #         close_metric = torch.mean(metric[name], dim=(1, 2, 3))
    ...
    return train_loss/(batch_idx+1), 100.*correct/total, open_sub, close_sub
    ...
    for i in range(args.max_epoch):
        ...
        for client_idx in range(client_num):
            if i < args.pretrain_epoch:
                ...
                train_loss, _, open_metric, close_metric = traindummy(args, trainloaders[client_idx], models[client_idx], optimizers[client_idx], criterion)
            
            if i == args.pretrain_epoch:
                np.save(os.path.join('/data/cyang/Code/Experiment/visual', '{}_open.npy'.format(datasets[client_idx])), open_metric.cpu().numpy())
                np.save(os.path.join('/data/cyang/Code/Experiment/visual', '{}_close.npy'.format(datasets[client_idx])), close_metric.cpu().numpy())
